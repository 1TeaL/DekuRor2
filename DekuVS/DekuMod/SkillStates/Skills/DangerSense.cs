...
namespace DekuMod.SkillStates
{
    public class DangerSense : BaseSkillState
    {
        ...
        public override void OnEnter()
        {
            ...
            this.fireTime = duration / (4 * attackSpeedStat * fajin);
            ...
            base.PlayAnimation("LeftArm, Override", "FingerFlick", "Attack.playbackRate", this.fireTime);
        ...
        {
            base.FixedUpdate(); 
            if(base.fixedAge >= fireTime && !base.characterBody.HasBuff(Modules.Buffs.counterBuff.buffIndex) && !counteron)
            {
                counteron = true;
                bool active = NetworkServer.active;
                if (active)
            ...
            if (base.fixedAge >= fireTime && base.fixedAge <(baseDuration - fireTime) && !hasFired && !base.characterBody.HasBuff(Modules.Buffs.counterBuff.buffIndex))
            {
                ...
                if (base.isAuthority)
                {
                    ...
                    if (dekucon.isMaxPower)
                    {
                        ...
                        damageType = DamageType.BypassArmor | DamageType.Freeze2s;
                    }
                    else
                    {
                    ...
                    //base.PlayAnimation("Fullbody, Override", "ShootStyleFullFlip", "Attack.playbackRate", this.duration / (4 * attackSpeedStat * fajin));
                    //base.PlayCrossfade("FullBody, Override", "ShootStyleKickExit", "Attack.playbackRate", this.duration / (4 * attackSpeedStat * fajin), this.duration / (4 * attackSpeedStat * fajin));
            ...
            {
                if (base.fixedAge >= (baseDuration - fireTime))
                {
                    bool active = NetworkServer.active;
                    if (active && base.characterBody.HasBuff(Modules.Buffs.counterBuff))
                    {
