...
{
    internal class PerformBlackwhipPullNetworkRequest : INetMessage
    {
        //Network these ones.
        ...
        public PerformBlackwhipPullNetworkRequest()
        {

        }

        public PerformBlackwhipPullNetworkRequest(NetworkInstanceId netID, Vector3 origin, Vector3 direction, float pullRange)
        {
            this.netID = netID;
            this.origin = origin;
            ...
            if (trackingTargets.Count > 0)
            {
                foreach (HurtBox singularTarget in trackingTargets)
                {
                    ...
                    if (singularTarget.healthComponent && singularTarget.healthComponent.body)
                    {
                        ...
                        {
                            attacker = bodyObj,
                            damage = charBody.damage * Modules.StaticValues.blackwhip100DamageCoefficient,
        ...
        private void SearchForTarget(CharacterBody charBody)
        {
            ...
            this.search.maxDistanceFilter = StaticValues.blackwhip100PullRange;
            this.search.maxAngleFilter = 90f;
